...
} from '../utils/worldInteraction.js'
...
export function createWorldInteractionStoreDefinition(api) {
  const {
    createWorldRound,
    executeWorldRound,
    getActiveWorldRound,
  ...
  actions: {
    ...
    async finishRound(round, generation = this.generation) {
      const finishingWorldId = this.worldId
      const finishingRoundId = round?.id
      ...
      if (generation !== this.generation || String(this.worldId) !== String(finishingWorldId)
        || String(this.activeRound?.id) !== String(finishingRoundId)) return
