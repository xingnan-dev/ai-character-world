<template>
  <div class="world-page">
    ...
    <header class="page-header">
      ...
      <el-button type="primary" size="large" @click="router.push('/worlds/create')">＋ 创建世界</el-button>
    </header>
    <main v-loading="worldStore.loading">
      ...
      <el-empty v-else-if="!worldStore.loading && worldStore.list.length === 0" description="还没有创建世界">
        <el-button type="primary" @click="router.push('/worlds/create')">描述第一个世界</el-button>
      </el-empty>
      <div v-else class="world-grid">
        <article v-for="world in worldStore.list" :key="world.id" class="world-card" :class="accentForWorld(world.id)" @click="open(world.id)">
          ...
          <div class="card-actions"><button @click.stop="open(world.id)">查看详情 →</button><button class="delete" @click.stop="remove(world)">删除</button></div>
          </div>
        </article>
      </div>
    </main>
